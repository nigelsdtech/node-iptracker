    ...
    jsonFile.readFile(file, function(err, oldIPs) {
      ...
      if (err) {
        ...
        if (err.code == "ENOENT") {
          ...
          log.error("Notifying of error loading old IP file: " + err)
          reporter.handleError({errMsg: "Error loading old IP file: " + err})
        }
      }
  ...
  .spread (function (extIP, intIP, oldIPs) {
    ...
    if (changes) {
      ...
      jsonFile.writeFile(cfg.ipStoreFile, newIPs , function (err) {
        if (err) {
          log.error("Notifying of error writing to IP file: " + err)
          reporter.handleError({errMsg: "Error writing to IP file: " + err})
          return
        }
